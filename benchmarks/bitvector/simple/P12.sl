;; test if number of leadering zeroes on first input is less than or equal to that of the second input
...
;;(constraint (Start.Sem bvformula #x000ff000 #x0000ffff true ))
;;(constraint (Start.Sem bvformula #x0000ffff #x10000000 false ))
(constraint (forall ((x (_ BitVec 32)) (y (_ BitVec 32)) (result Bool))
            (= (Start.Sem bvformula x y result)
                (= result (bvule (bvand x (bvnot y)) y) ))))
